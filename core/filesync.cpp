		...
		else
		{
			try
			{
				...
				if (need_upload)
				{
					...
					{
						on_file_uploaded(std::string(path), md5);
					}
				}
			}
			catch (const std::exception &e)
...
bool filesync::FileSync::upload_file(std::shared_ptr<std::istream> fs, const char *md5, long size, std::string token)
{
	XTCP::message reply;
	...
	auto file_size = std::stoul(data["size"].get<std::string>());
	auto uploaded_size = std::stoul(data["uploaded_size"].get<std::string>());
	...
	if (!*fs)
	{
		common::print_info(common::string_format("abnormal file stream"));
		return false;
	}
	common::print_info(common::string_format("uploading file %s...", "unknown"));
	size_t written = 0;
